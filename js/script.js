...
$("#usermail input").focusout(function() {
  ...
  // 3. 반화번호를 형식에 맞지 않게 입력했을 경우 인증번호 입력탄 비활성화
  ...
  // $()부터 disabled이라는 클래스 remove
    if(veri1 && veri2) {
      ...
      $("#veritext").attr("disabled");
      // #veritext에 disabled
      $("#veritext").parent('.inputbox').addClass('.disinput');
      // #veritext ID를 가진 parent .inputbox에 .disinput 클래스 추가(인증번호 입력칸 비활성화)
    }

    
  })

// #veritext에서 focusout됐을 때 그 값이 "1234"와 같다면 (조건1)
// "인증되었습니다." text-green (실행문1)
    ...
    // Criate, Read, Update, Delete -> API의 기본요소 4가지
    // API를 만드는 역할을 하는게 서버 개발자. 즉 백앤드 개발자
...
// #joinbtn을 눌렀을 때 필수요소가 true라면 (조건1)
